...
class QuadricSimplifyMesh {
  constructor(vs, ts, targetCount, aggressiveness = 7, finishLossless = false, verbose = true) {
    this.vertices = []
    this.triangles = []
    this.refs = Array.from({ length: ts.length }) // Pre-allocate memory
  ...
  simplify(verbose = true) {
    ...
    const deleted0 = Array.from({ length: triangleCount * 3 }).fill(false) // overprovision
    const deleted1 = Array.from({ length: triangleCount * 3 }).fill(false) // overprovision
    ...
    for (let iteration = 0; iteration < maxIter; iteration++) {
      ...
      if (lossy) {
        ...
        threshold = 0.000000001 * (iteration + 3.0) ** this.aggressiveness
      }
      else {
    ...
    let j = 0
    for (j = 0; j < finalTs.length; j++) {
      finalTs[j] = j
    }

    const finalVs = new Float32Array(this.triangles.length * 9)
    let k = 0
    for (const triangle of this.triangles) {
      finalVs[k++] = this.vertices[triangle.v[0]].p.x
      finalVs[k++] = this.vertices[triangle.v[0]].p.y
      finalVs[k++] = this.vertices[triangle.v[0]].p.z
      finalVs[k++] = this.vertices[triangle.v[1]].p.x
      finalVs[k++] = this.vertices[triangle.v[1]].p.y
      finalVs[k++] = this.vertices[triangle.v[1]].p.z
      finalVs[k++] = this.vertices[triangle.v[2]].p.x
      finalVs[k++] = this.vertices[triangle.v[2]].p.y
      finalVs[k++] = this.vertices[triangle.v[2]].p.z
    }
    if (verbose) {
      // const pct = Math.round((100 * (finalTs.length / 3)) / triangleCount)
      // console.log(`Vertices: ${finalVs.length / 3}, Triangles: ${finalTs.length / 3}, Percent: ${pct}%`)
    }
...
export function simplifyJS(verts, tris, tri_fraction = 0.5, aggressiveness = 7, finishLossless = false, verbose = false) {
  // console.log('simplifier','verts', verts, 'tris', tris, 'tri_fraction', tri_fraction, 'aggressiveness', aggressiveness, 'finishLossless', finishLossless, 'verbose', verbose);
